...
class Game:
    ...
    async def forward_msg(self, msg: Message, player: Player):
        ...
        if self.has_finished():
            raise RuntimeError(
                f"You are trying to handle a message for a game which has finished")
        ...
        if not self.is_playing(player):
            raise ValueError(
                f"{player.socket.peername} is not playing in the game")
        ...
        # TODO: a lock is needed. If the same player sends two messages, then it might be possible that both of them
        # enter here, when it shouldn't
    ...
    def has_finished(self) -> bool:
        ...
        return self._finished_cnt == 2 or self._errors_cnt == self.max_consecutive_errors
